function [output] = get_VOI(B, all_mods, q, pr_prob, c1, c2, Start_m1)
...
%   output[:,19] = Exp 2 species monitoring for species 2 & 3, managing threat 2
    ...
    for r = 1:length(B)
        evpi1(r) = get_EVPI_1(all_mods, mod_prob_all, B(r), c1, c2, q);
        out_evpi2 = get_EVPI_2(all_mods, mod_prob_all, B(r), c1, c2, q, Start_m1);
        evpi2(r) = out_evpi2(1);
    end
    evpi = - evpi1 + evpi2;
    ...
    for sp_monit = 1:3
        ...
        for r = 1:length(B)
            evsi1(1,r) = get_EVSI_1(all_mods, mod_prob_all, B(r), c1, c2,...
                q,Start_m1, sp_monit);
            out_evpi2 = get_EVPI_2(all_mods, mod_prob_all, B(r), c1, c2,...
                q, Start_m1);
    ...
    for monitor = 1:3
        ...
        evsi2 = NaN([1,length(B)]);
        for r = 1:length(B)
            evsi1(1,r) = get_EVSI_1_2sp(all_mods, mod_prob_all, B(r), c1,...
    ...
    for a = 1:2
        ...
        for sp_monit = 1:3
            ...
            evsi2 = NaN([1,length(B)]);
            for r = 1:length(B)
                evsi1(1,r) = get_EVSI_1_exp(all_mods, managed_mods,...
                    mod_prob_all, B(r), c1, c2, q, Start_m1, sp_monit);
                out_evpi2 = get_EVPI_2(all_mods, mod_prob_all, B(r), c1,...
                    c2, q, Start_m1);
                evsi2(1,r) = out_evpi2(1);
            end
